...
fn ldMatrixBf16(comptime regs: usize, comptime trans: bool, src: [*]addrspace(.shared) const u16) @Vector(regs, u32) {
...
fn mmaM16N8K16(a: U32x4, b: @Vector(2, u32), c: @Vector(4, f32)) @Vector(4, f32) {
    ...
    return @bitCast(f(a[0], a[1], a[2], a[3], b[0], b[1], c[0], c[1], c[2], c[3]));
}
...
fn doMatmul(
    ...
    while (k_block < k) : (k_block += block_k) {
        ...
        inline for (0..2) |m_i| {
            ...
            const a_frag = ldMatrixBf16(4, false, @ptrCast(@alignCast(&a_shared[a_row * 2 + a_col])));
            ...
            inline for (0..2) |n_i| {
                ...
                const b_frag = ldMatrixBf16(2, false, @ptrCast(@alignCast(&b_shared[b_row * 2 + b_matrix])));
    ...
    inline for (0..2) |m_i| {
        ...
        inline for (0..2) |n_i| {
            ...
            c[row0 * n + col + 0] = f32ToBf16Bits(c_frag[m_i][n_i][0]);
            c[row0 * n + col + 1] = f32ToBf16Bits(c_frag[m_i][n_i][1]);
            c[row1 * n + col + 0] = f32ToBf16Bits(c_frag[m_i][n_i][2]);
            c[row1 * n + col + 1] = f32ToBf16Bits(c_frag[m_i][n_i][3]);
        }
    }
}
